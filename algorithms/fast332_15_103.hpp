...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 1, num_threads)) untied
    ...
    FastMatmulRecursive(A13, T1, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 15, x, num_threads);
    T1.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 3, num_threads)) untied
    ...
    T_Add3(B21, B22, T3, x);
    FastMatmulRecursive(S3, T3, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 15, x, num_threads);
    S3.deallocate();
    T3.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 6, num_threads)) untied
    ...
    FastMatmulRecursive(S6, B11, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 15, x, num_threads);
    S6.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 8, num_threads)) untied
    ...
    FastMatmulRecursive(S8, B12, M8, total_steps, steps_left - 1, (start_index + 8 - 1) * 15, x, num_threads);
    S8.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 9, num_threads)) untied
    ...
    T_Add9(B21, B31, T9, x);
    FastMatmulRecursive(A32, T9, M9, total_steps, steps_left - 1, (start_index + 9 - 1) * 15, x, num_threads);
    T9.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 12, num_threads)) untied
    ...
    FastMatmulRecursive(S12, B31, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 15, x, num_threads);
    S12.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 13, num_threads)) untied
    ...
    FastMatmulRecursive(A11, T13, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 15, x, num_threads);
    T13.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 14, num_threads)) untied
    ...
    FastMatmulRecursive(A31, T14, M14, total_steps, steps_left - 1, (start_index + 14 - 1) * 15, x, num_threads);
    T14.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
