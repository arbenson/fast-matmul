...
# pragma omp task if(sequential1) shared(mem_mngr)
    ...
    S_Add1(A21, A22, A31, A32, S1, x, sequential1);
    FastMatmulRecursive(mem_mngr, S1, B12, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential2) shared(mem_mngr)
    ...
    S_Add2(A12, A22, A32, S2, x, sequential2);
    Matrix<Scalar> T2(mem_mngr.GetMem(start_index, 2, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
    T_Add2(B21, B22, T2, x, sequential2);
    FastMatmulRecursive(mem_mngr, S2, T2, M2, total_steps, steps_left - 1, (start_index + 2 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential3) shared(mem_mngr)
    ...
    T_Add3(B11, B12, T3, x, sequential3);
    FastMatmulRecursive(mem_mngr, S3, T3, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential4) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A41, T4, M4, total_steps, steps_left - 1, (start_index + 4 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential5) shared(mem_mngr)
    ...
    S_Add5(A32, A41, S5, x, sequential5);
    Matrix<Scalar> T5(mem_mngr.GetMem(start_index, 5, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
    T_Add5(B12, B21, T5, x, sequential5);
    FastMatmulRecursive(mem_mngr, S5, T5, M5, total_steps, steps_left - 1, (start_index + 5 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential6) shared(mem_mngr)
    ...
    S_Add6(A32, A42, S6, x, sequential6);
    Matrix<Scalar> T6(mem_mngr.GetMem(start_index, 6, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
    T_Add6(B21, B22, T6, x, sequential6);
    FastMatmulRecursive(mem_mngr, S6, T6, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential7) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A11, T7, M7, total_steps, steps_left - 1, (start_index + 7 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential8) shared(mem_mngr)
    ...
    S_Add8(A11, A21, A41, S8, x, sequential8);
    Matrix<Scalar> T8(mem_mngr.GetMem(start_index, 8, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
    T_Add8(B11, B12, T8, x, sequential8);
    FastMatmulRecursive(mem_mngr, S8, T8, M8, total_steps, steps_left - 1, (start_index + 8 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential9) shared(mem_mngr)
    ...
    S_Add9(A11, A22, A32, S9, x, sequential9);
    Matrix<Scalar> T9(mem_mngr.GetMem(start_index, 9, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
    T_Add9(B12, B21, T9, x, sequential9);
    FastMatmulRecursive(mem_mngr, S9, T9, M9, total_steps, steps_left - 1, (start_index + 9 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential10) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, S10, B21, M10, total_steps, steps_left - 1, (start_index + 10 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential11) shared(mem_mngr)
    ...
    S_Add11(A31, A32, S11, x, sequential11);
    FastMatmulRecursive(mem_mngr, S11, B12, M11, total_steps, steps_left - 1, (start_index + 11 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential12) shared(mem_mngr)
    ...
    S_Add12(A41, A42, S12, x, sequential12);
    FastMatmulRecursive(mem_mngr, S12, B21, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential13) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A32, T13, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential14) shared(mem_mngr)
    ...
    T_Add14(B12, B22, T14, x, sequential14);
    FastMatmulRecursive(mem_mngr, A22, T14, M14, total_steps, steps_left - 1, (start_index + 14 - 1) * 14, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp taskwait
    ...
    M_Add1(M7, M10, C11, x, false, beta);
    M_Add2(M2, M9, M10, M13, M14, C12, x, false, beta);
    M_Add3(M1, M4, M5, M7, M8, M9, M11, C21, x, false, beta);
    M_Add4(M1, M11, M14, C22, x, false, beta);
    M_Add5(M3, M4, M5, M8, M11, C31, x, false, beta);
    M_Add6(M11, M13, C32, x, false, beta);
    M_Add7(M4, M12, C41, x, false, beta);
    M_Add8(M5, M6, M12, M13, C42, x, false, beta);
...
double FastMatmul(Matrix<Scalar>& A, Matrix<Scalar>& B, Matrix<Scalar>& C,
    ...
    mem_mngr.Allocate(4, 2, 2, 14, num_steps, A.m(), A.n(), B.n());
