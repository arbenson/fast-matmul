...
template <typename Scalar>
void M_Add3(Matrix<Scalar>& M1, Matrix<Scalar>& M2, Matrix<Scalar>& M3, Matrix<Scalar>& M4, Matrix<Scalar>& M5, Matrix<Scalar>& M6, Matrix<Scalar>& M7, Matrix<Scalar>& C, double x=1e-8) {
    const int strideM1 = M1.stride();
    const int strideM2 = M2.stride();
    const int strideM3 = M3.stride();
    const int strideM4 = M4.stride();
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 1, num_threads)) untied
    ...
    FastMatmulRecursive(S1, B12, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 14, x, num_threads);
    S1.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 2, num_threads)) untied
    ...
    T_Add2(B21, B22, T2, x);
    FastMatmulRecursive(S2, T2, M2, total_steps, steps_left - 1, (start_index + 2 - 1) * 14, x, num_threads);
    S2.deallocate();
    T2.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 3, num_threads)) untied
    ...
    T_Add3(B11, B12, T3, x);
    FastMatmulRecursive(S3, T3, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 14, x, num_threads);
    S3.deallocate();
    T3.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 4, num_threads)) untied
    ...
    T_Add4(B11, B21, T4, x);
    FastMatmulRecursive(A41, T4, M4, total_steps, steps_left - 1, (start_index + 4 - 1) * 14, x, num_threads);
    T4.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 5, num_threads)) untied
    ...
    T_Add5(B12, B21, T5, x);
    FastMatmulRecursive(S5, T5, M5, total_steps, steps_left - 1, (start_index + 5 - 1) * 14, x, num_threads);
    S5.deallocate();
    T5.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 6, num_threads)) untied
    ...
    T_Add6(B21, B22, T6, x);
    FastMatmulRecursive(S6, T6, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 14, x, num_threads);
    S6.deallocate();
    T6.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 7, num_threads)) untied
    ...
    T_Add7(B11, B21, T7, x);
    FastMatmulRecursive(A11, T7, M7, total_steps, steps_left - 1, (start_index + 7 - 1) * 14, x, num_threads);
    T7.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 8, num_threads)) untied
    ...
    T_Add8(B11, B12, T8, x);
    FastMatmulRecursive(S8, T8, M8, total_steps, steps_left - 1, (start_index + 8 - 1) * 14, x, num_threads);
    S8.deallocate();
    T8.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 9, num_threads)) untied
    ...
    T_Add9(B12, B21, T9, x);
    FastMatmulRecursive(S9, T9, M9, total_steps, steps_left - 1, (start_index + 9 - 1) * 14, x, num_threads);
    S9.deallocate();
    T9.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 10, num_threads)) untied
    ...
    FastMatmulRecursive(S10, B21, M10, total_steps, steps_left - 1, (start_index + 10 - 1) * 14, x, num_threads);
    S10.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 11, num_threads)) untied
    ...
    FastMatmulRecursive(S11, B12, M11, total_steps, steps_left - 1, (start_index + 11 - 1) * 14, x, num_threads);
    S11.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 12, num_threads)) untied
    ...
    FastMatmulRecursive(S12, B21, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 14, x, num_threads);
    S12.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 13, num_threads)) untied
    ...
    FastMatmulRecursive(A32, T13, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 14, x, num_threads);
    T13.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(14, total_steps, steps_left, start_index, 14, num_threads)) untied
    ...
    FastMatmulRecursive(A22, T14, M14, total_steps, steps_left - 1, (start_index + 14 - 1) * 14, x, num_threads);
    T14.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp taskwait
    ...
    M_Add1(M7, M10, C11, x);
    M_Add2(M2, M9, M10, M13, M14, C12, x);
    M_Add3(M1, M4, M5, M7, M8, M9, M11, C21, x);
    M_Add4(M1, M11, M14, C22, x);
    M_Add5(M3, M4, M5, M8, M11, C31, x);
    M_Add6(M11, M13, C32, x);
    M_Add7(M4, M12, C41, x);
    M_Add8(M5, M6, M12, M13, C42, x);
