...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 1, num_threads)) untied
    ...
    FastMatmulRecursive(A12, B_X1, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 23, x, num_threads);
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 3, num_threads)) untied
    ...
    FastMatmulRecursive(A_X1, T3, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 23, x, num_threads);
    T3.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 5, num_threads)) untied
    ...
    FastMatmulRecursive(S5, B_X2, M5, total_steps, steps_left - 1, (start_index + 5 - 1) * 23, x, num_threads);
    S5.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 6, num_threads)) untied
    ...
    FastMatmulRecursive(S6, B_X4, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 23, x, num_threads);
    S6.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 9, num_threads)) untied
    ...
    FastMatmulRecursive(A_X5, B_X5, M9, total_steps, steps_left - 1, (start_index + 9 - 1) * 23, x, num_threads);
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 12, num_threads)) untied
    ...
    FastMatmulRecursive(S12, B_X6, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 23, x, num_threads);
    S12.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 13, num_threads)) untied
    ...
    FastMatmulRecursive(S13, B_X3, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 23, x, num_threads);
    S13.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 15, num_threads)) untied
    ...
    FastMatmulRecursive(A_X6, B31, M15, total_steps, steps_left - 1, (start_index + 15 - 1) * 23, x, num_threads);
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 16, num_threads)) untied
    ...
    FastMatmulRecursive(A_X5, B11, M16, total_steps, steps_left - 1, (start_index + 16 - 1) * 23, x, num_threads);
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 20, num_threads)) untied
    ...
    FastMatmulRecursive(A_X2, T20, M20, total_steps, steps_left - 1, (start_index + 20 - 1) * 23, x, num_threads);
    T20.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(23, total_steps, steps_left, start_index, 22, num_threads)) untied
    ...
    FastMatmulRecursive(S22, B_X1, M22, total_steps, steps_left - 1, (start_index + 22 - 1) * 23, x, num_threads);
    S22.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp taskwait
    ...
    M_Add2(M9, M12, M16, M18, M_X1, C12, x);
    M_Add3(M5, M8, M19, M_X2, C13, x);
    M_Add4(M15, M22, M23, M_X3, M_X4, C21, x);
    M_Add5(M10, M18, M20, M_X1, C22, x);
    M_Add6(M6, M10, M11, M13, M_X1, C23, x);
    M_Add7(M4, M23, M_X3, M_X4, C31, x);
    M_Add8(M4, M9, M20, M_X3, M_X5, C32, x);
    M_Add9(M11, M_X2, M_X5, C33, x);
