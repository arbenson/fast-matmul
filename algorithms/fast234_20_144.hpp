...
# pragma omp task if(sequential3) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A13, T3, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 20, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential5) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, S5, B34, M5, total_steps, steps_left - 1, (start_index + 5 - 1) * 20, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential10) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A22, T10, M10, total_steps, steps_left - 1, (start_index + 10 - 1) * 20, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential13) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, S13, B14, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 20, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential14) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A11, T14, M14, total_steps, steps_left - 1, (start_index + 14 - 1) * 20, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential15) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A23, T15, M15, total_steps, steps_left - 1, (start_index + 15 - 1) * 20, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
