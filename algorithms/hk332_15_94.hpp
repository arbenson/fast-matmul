...
# pragma omp task if(sequential1) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, S1, B12, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential3) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, S3, B21, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential4) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A21, T4, M4, total_steps, steps_left - 1, (start_index + 4 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential6) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A31, T6, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential7) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, S7, B31, M7, total_steps, steps_left - 1, (start_index + 7 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential10) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A12, T10, M10, total_steps, steps_left - 1, (start_index + 10 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential12) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, S12, B32, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential13) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A23, T13, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential15) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A13, T15, M15, total_steps, steps_left - 1, (start_index + 15 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
