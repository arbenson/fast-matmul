...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 1, num_threads)) untied
    ...
    FastMatmulRecursive(S1, B12, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 15, x, num_threads);
    S1.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 2, num_threads)) untied
    ...
    T_Add2(B11, B12, T2, x);
    FastMatmulRecursive(S2, T2, M2, total_steps, steps_left - 1, (start_index + 2 - 1) * 15, x, num_threads);
    S2.deallocate();
    T2.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 3, num_threads)) untied
    ...
    FastMatmulRecursive(S3, B21, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 15, x, num_threads);
    S3.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 4, num_threads)) untied
    ...
    T_Add4(B11, B21, T4, x);
    FastMatmulRecursive(A21, T4, M4, total_steps, steps_left - 1, (start_index + 4 - 1) * 15, x, num_threads);
    T4.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 6, num_threads)) untied
    ...
    FastMatmulRecursive(A31, T6, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 15, x, num_threads);
    T6.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 7, num_threads)) untied
    ...
    FastMatmulRecursive(S7, B31, M7, total_steps, steps_left - 1, (start_index + 7 - 1) * 15, x, num_threads);
    S7.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 9, num_threads)) untied
    ...
    T_Add9(B21, B22, T9, x);
    FastMatmulRecursive(S9, T9, M9, total_steps, steps_left - 1, (start_index + 9 - 1) * 15, x, num_threads);
    S9.deallocate();
    T9.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 10, num_threads)) untied
    ...
    FastMatmulRecursive(A12, T10, M10, total_steps, steps_left - 1, (start_index + 10 - 1) * 15, x, num_threads);
    T10.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 11, num_threads)) untied
    ...
    FastMatmulRecursive(A32, T11, M11, total_steps, steps_left - 1, (start_index + 11 - 1) * 15, x, num_threads);
    T11.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 12, num_threads)) untied
    ...
    FastMatmulRecursive(S12, B32, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 15, x, num_threads);
    S12.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 13, num_threads)) untied
    ...
    FastMatmulRecursive(A23, T13, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 15, x, num_threads);
    T13.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(15, total_steps, steps_left, start_index, 15, num_threads)) untied
    ...
    FastMatmulRecursive(A13, T15, M15, total_steps, steps_left - 1, (start_index + 15 - 1) * 15, x, num_threads);
    T15.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp taskwait
    ...
    M_Add1(M1, M2, M4, M5, M6, M8, C11, x);
    M_Add2(M1, M6, M8, M10, M15, C12, x);
    M_Add3(M3, M4, M11, M13, M14, C21, x);
    M_Add4(M3, M5, M9, M10, M11, M14, C22, x);
    M_Add5(M6, M7, M11, M13, M14, C31, x);
    M_Add6(M6, M8, M11, M12, M15, C32, x);
