...
# pragma omp task if(sequential1) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, S1, B21, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential2) shared(mem_mngr)
    ...
    T_Add2(B11, B21, T2, x, sequential2);
    FastMatmulRecursive(mem_mngr, A11, T2, M2, total_steps, steps_left - 1, (start_index + 2 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential3) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, S3, B12, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential6) shared(mem_mngr)
    ...
    S_Add6(A11, A12, A31, A32, S6, x, sequential6);
    Matrix<Scalar> T6(mem_mngr.GetMem(start_index, 6, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
    T_Add6(B11, B13, T6, x, sequential6);
    FastMatmulRecursive(mem_mngr, S6, T6, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential7) shared(mem_mngr)
    {
#endif
    FastMatmulRecursive(mem_mngr, A31, B13, M7, total_steps, steps_left - 1, (start_index + 7 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential8) shared(mem_mngr)
    ...
    S_Add8(A11, A12, A32, S8, x, sequential8);
    Matrix<Scalar> T8(mem_mngr.GetMem(start_index, 8, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
    T_Add8(B11, B13, B21, T8, x, sequential8);
    FastMatmulRecursive(mem_mngr, S8, T8, M8, total_steps, steps_left - 1, (start_index + 8 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential9) shared(mem_mngr)
    ...
    FastMatmulRecursive(mem_mngr, A22, T9, M9, total_steps, steps_left - 1, (start_index + 9 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential11) shared(mem_mngr)
    ...
    S_Add11(A21, A22, A31, A32, S11, x, sequential11);
...
# pragma omp task if(sequential12) shared(mem_mngr)
    {
#endif
    FastMatmulRecursive(mem_mngr, A32, B23, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential13) shared(mem_mngr)
    ...
    T_Add13(B12, B13, B22, B23, T13, x, sequential13);
    FastMatmulRecursive(mem_mngr, S13, T13, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential14) shared(mem_mngr)
    ...
    S_Add14(A21, A22, A31, S14, x, sequential14);
    Matrix<Scalar> T14(mem_mngr.GetMem(start_index, 14, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
    T_Add14(B12, B22, B23, T14, x, sequential14);
    FastMatmulRecursive(mem_mngr, S14, T14, M14, total_steps, steps_left - 1, (start_index + 14 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential15) shared(mem_mngr)
    ...
    T_Add15(B11, B13, B21, B23, T15, x, sequential15);
    FastMatmulRecursive(mem_mngr, S15, T15, M15, total_steps, steps_left - 1, (start_index + 15 - 1) * 15, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
