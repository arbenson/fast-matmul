#ifndef _fast423_156_HPP_
#define _fast423_156_HPP_

// This is an automatically generated file from gen.py.
#include "common.hpp"

namespace fast423_156 {
...
template <typename Scalar>
void S_Add19(Matrix<Scalar>& S1, Matrix<Scalar>& S2, Matrix<Scalar>& S3, Matrix<Scalar>& S4, Matrix<Scalar>& S5, Matrix<Scalar>& S6, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideS1 = S1.stride();
    const int strideS2 = S2.stride();
    const int strideS3 = S3.stride();
    const int strideS4 = S4.stride();
...
template <typename Scalar>
void S_Add20(Matrix<Scalar>& S1, Matrix<Scalar>& S2, Matrix<Scalar>& S3, Matrix<Scalar>& S4, Matrix<Scalar>& S5, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideS1 = S1.stride();
    const int strideS2 = S2.stride();
    const int strideS3 = S3.stride();
    const int strideS4 = S4.stride();
...
template <typename Scalar>
void T_Add20(Matrix<Scalar>& T1, Matrix<Scalar>& T2, Matrix<Scalar>& T3, Matrix<Scalar>& T4, Matrix<Scalar>& T5, Matrix<Scalar>& C, double x, bool sequential) {
    const int strideT1 = T1.stride();
    const int strideT2 = T2.stride();
    const int strideT3 = T3.stride();
    const int strideT4 = T4.stride();
...
template <typename Scalar>
void M_Add11(Matrix<Scalar>& M1, Matrix<Scalar>& M2, Matrix<Scalar>& M3, Matrix<Scalar>& M4, Matrix<Scalar>& M5, Matrix<Scalar>& M6, Matrix<Scalar>& M7, Matrix<Scalar>& M8, Matrix<Scalar>& M9, Matrix<Scalar>& M10, Matrix<Scalar>& C, double x, bool sequential, Scalar beta) {
    const int strideM1 = M1.stride();
    const int strideM2 = M2.stride();
    const int strideM3 = M3.stride();
    const int strideM4 = M4.stride();
...
# pragma omp task if(sequential1) shared(mem_mngr, locker) untied
    ...
    M1.UpdateMultiplier(Scalar(-1));
    FastMatmulRecursive(locker, mem_mngr, A41, B13, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 20, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
...
# pragma omp task if(sequential2) shared(mem_mngr, locker) untied
    ...
    M2.UpdateMultiplier(Scalar(1));
    FastMatmulRecursive(locker, mem_mngr, A11, B12, M2, total_steps, steps_left - 1, (start_index + 2 - 1) * 20, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
...
# pragma omp task if(sequential3) shared(mem_mngr, locker) untied
    ...
    M3.UpdateMultiplier(Scalar(1));
    FastMatmulRecursive(locker, mem_mngr, A11, B13, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 20, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
...
#endif
    S_Add4(A12, A22, A32, S4, x, sequential4);
    M4.UpdateMultiplier(Scalar(-1));
    FastMatmulRecursive(locker, mem_mngr, S4, B22, M4, total_steps, steps_left - 1, (start_index + 4 - 1) * 20, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S4.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
# endif
    ...
    // M5 = (-1 * A21 + 1 * A22 + 1 * A41 + -1 * A42) * (1 * B11 + 1 * B13)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add6(A21, A22, S6, x, sequential6);
...
# endif
    ...
    // M7 = (1 * A22 + -1 * A42) * (1 * B11 + 1 * B13 + 1 * B21 + 1 * B23)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
# endif
    ...
    // M8 = (1 * A21 + 1 * A32) * (1 * B11 + 1 * B12 + 1 * B21)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
# endif
    ...
    // M10 = (1 * A21 + -1 * A22 + 1 * A42) * (1 * B11 + 1 * B13 + 1 * B21)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add10(B11, B13, B21, T10, x, sequential10);
...
# pragma omp task if(sequential11) shared(mem_mngr, locker) untied
    ...
    M11.UpdateMultiplier(Scalar(1));
    FastMatmulRecursive(locker, mem_mngr, A12, B22, M11, total_steps, steps_left - 1, (start_index + 11 - 1) * 20, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
...
# pragma omp task if(sequential12) shared(mem_mngr, locker) untied
    ...
    M12.UpdateMultiplier(Scalar(1));
    FastMatmulRecursive(locker, mem_mngr, A42, B23, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 20, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
locker.Decrement();
...
# endif
    ...
    // M13 = (1 * A11 + -1 * A31 + 1 * A32 + -1 * A41) * (1 * B11 + 1 * B12 + 1 * B21 + 1 * B22 + 1 * B23)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add14(B11, B21, T14, x, sequential14);
    M14.UpdateMultiplier(Scalar(-1));
    FastMatmulRecursive(locker, mem_mngr, A21, T14, M14, total_steps, steps_left - 1, (start_index + 14 - 1) * 20, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T14.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
# endif
    ...
    // M15 = (-1 * A11 + 1 * A31 + 1 * A41) * (1 * B11 + 1 * B12 + 1 * B13 + 1 * B21 + 1 * B22 + 1 * B23)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
# endif
    ...
    // M16 = (-1 * A32) * (1 * B11 + 1 * B12 + 1 * B21 + 1 * B22)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    T_Add16(B11, B12, B21, B22, T16, x, sequential16);
    M16.UpdateMultiplier(Scalar(-1));
    FastMatmulRecursive(locker, mem_mngr, A32, T16, M16, total_steps, steps_left - 1, (start_index + 16 - 1) * 20, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    T16.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
#endif
    S_Add17(A21, A31, S17, x, sequential17);
...
# pragma omp task if(sequential18) shared(mem_mngr, locker) untied
    ...
    FastMatmulRecursive(locker, mem_mngr, S18, B23, M18, total_steps, steps_left - 1, (start_index + 18 - 1) * 20, x, num_threads, Scalar(0.0));
#ifndef _PARALLEL_
    S18.deallocate();
#endif
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
# endif
    ...
    // M19 = (1 * A11 + -1 * A12 + -1 * A31 + 1 * A32 + -1 * A41 + 1 * A42) * (-1 * B21 + -1 * B22 + -1 * B23)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
# endif
    ...
    // M20 = (-1 * A11 + 1 * A22 + 1 * A31 + 1 * A41 + -1 * A42) * (1 * B11 + 1 * B13 + 1 * B21 + 1 * B22 + 1 * B23)
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
...
# pragma omp taskwait
    ...
    M_Add1(M1, M4, M5, M6, M7, M8, M9, M10, M13, M14, M15, M16, M17, M18, M19, M20, C11, x, false, beta);
    M_Add2(M2, M11, C12, x, false, beta);
    M_Add3(M3, M12, M18, C13, x, false, beta);
    M_Add4(M6, M14, C21, x, false, beta);
    M_Add5(M4, M8, M11, M14, M16, C22, x, false, beta);
    M_Add6(M7, M10, M12, M14, C23, x, false, beta);
    M_Add7(M8, M9, M14, M17, C31, x, false, beta);
    M_Add8(M8, M14, M16, M17, C32, x, false, beta);
    M_Add9(M1, M3, M13, M15, M16, C33, x, false, beta);
    M_Add10(M1, M5, M6, M10, C41, x, false, beta);
    M_Add11(M2, M4, M7, M8, M11, M14, M15, M16, M17, M20, C42, x, false, beta);
    M_Add12(M1, M12, C43, x, false, beta);
