...
# pragma omp task if(sequential3) shared(mem_mngr) untied
    ...
    S_Add3(A11, A12, A31, A32, S3, x, sequential3);
...
# pragma omp task if(sequential4) shared(mem_mngr) untied
    ...
    T_Add4(B12, B13, T4, x, sequential4);
...
# pragma omp task if(sequential6) shared(mem_mngr) untied
    ...
    S_Add6(A22, A23, S6, x, sequential6);
    FastMatmulRecursive(mem_mngr, S6, B32, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 23, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential9) shared(mem_mngr) untied
    ...
    T_Add9(B12, B22, B32, T9, x, sequential9);
...
# pragma omp task if(sequential10) shared(mem_mngr) untied
    ...
    T_Add10(B12, B21, B22, T10, x, sequential10);
...
# pragma omp task if(sequential16) shared(mem_mngr) untied
    ...
    T_Add16(B22, B23, B32, B33, T16, x, sequential16);
...
# pragma omp task if(sequential17) shared(mem_mngr) untied
    ...
    S_Add17(A12, A13, A22, A23, S17, x, sequential17);
    Matrix<Scalar> T17(mem_mngr.GetMem(start_index, 17, total_steps - steps_left, T), B11.m(), B11.m(), B11.n());
    T_Add17(B22, B23, T17, x, sequential17);
    FastMatmulRecursive(mem_mngr, S17, T17, M17, total_steps, steps_left - 1, (start_index + 17 - 1) * 23, x, num_threads, Scalar(0.0));
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ || _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(sequential19) shared(mem_mngr) untied
    ...
    S_Add19(A11, A31, A32, S19, x, sequential19);
...
# pragma omp task if(sequential21) shared(mem_mngr) untied
    ...
    T_Add21(B12, B13, B33, T21, x, sequential21);
...
# pragma omp taskwait
    ...
    M_Add2(M3, M5, M6, M12, M17, M19, C12, x, false, beta);
    M_Add3(M3, M5, M19, M20, M21, C13, x, false, beta);
    M_Add4(M1, M2, M6, M7, M18, C21, x, false, beta);
    M_Add5(M6, M9, M22, C22, x, false, beta);
    M_Add6(M4, M9, M12, M16, M20, M21, M22, C23, x, false, beta);
    M_Add7(M1, M8, M11, C31, x, false, beta);
    M_Add8(M2, M5, M9, M10, M11, C32, x, false, beta);
    M_Add9(M5, M8, M14, M15, M19, M21, C33, x, false, beta);
