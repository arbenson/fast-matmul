...
# pragma omp task if(should_launch_task(7, total_steps, steps_left, start_index, 1, num_threads)) untied
    ...
    T_Add1(B11, B22, T1, x);
    FastMatmulRecursive(S1, T1, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 7, x, num_threads);
    S1.deallocate();
    T1.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(7, total_steps, steps_left, start_index, 2, num_threads)) untied
    ...
    S_Add2(A21, A22, S2, x);
    FastMatmulRecursive(S2, B11, M2, total_steps, steps_left - 1, (start_index + 2 - 1) * 7, x, num_threads);
    S2.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(7, total_steps, steps_left, start_index, 3, num_threads)) untied
    ...
    T_Add3(B12, B22, T3, x);
    FastMatmulRecursive(A11, T3, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 7, x, num_threads);
    T3.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(7, total_steps, steps_left, start_index, 4, num_threads)) untied
    ...
    T_Add4(B11, B21, T4, x);
    FastMatmulRecursive(A22, T4, M4, total_steps, steps_left - 1, (start_index + 4 - 1) * 7, x, num_threads);
    T4.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(7, total_steps, steps_left, start_index, 5, num_threads)) untied
    ...
    S_Add5(A11, A12, S5, x);
    FastMatmulRecursive(S5, B22, M5, total_steps, steps_left - 1, (start_index + 5 - 1) * 7, x, num_threads);
    S5.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(7, total_steps, steps_left, start_index, 6, num_threads)) untied
    ...
    T_Add6(B11, B12, T6, x);
    FastMatmulRecursive(S6, T6, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 7, x, num_threads);
    S6.deallocate();
    T6.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(7, total_steps, steps_left, start_index, 7, num_threads)) untied
    ...
    T_Add7(B21, B22, T7, x);
    FastMatmulRecursive(S7, T7, M7, total_steps, steps_left - 1, (start_index + 7 - 1) * 7, x, num_threads);
    S7.deallocate();
    T7.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp taskwait
    ...
    M_Add1(M1, M4, M5, M7, C11, x);
    M_Add2(M3, M5, C12, x);
    M_Add3(M2, M4, C21, x);
    M_Add4(M1, M2, M3, M6, C22, x);
