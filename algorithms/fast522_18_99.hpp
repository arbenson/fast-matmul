...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 1, num_threads)) untied
    ...
    FastMatmulRecursive(A32, B21, M1, total_steps, steps_left - 1, (start_index + 1 - 1) * 18, x, num_threads);
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 2, num_threads)) untied
    ...
    T_Add2(B11, B22, T2, x);
    FastMatmulRecursive(S2, T2, M2, total_steps, steps_left - 1, (start_index + 2 - 1) * 18, x, num_threads);
    S2.deallocate();
    T2.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 3, num_threads)) untied
    ...
    FastMatmulRecursive(A31, B11, M3, total_steps, steps_left - 1, (start_index + 3 - 1) * 18, x, num_threads);
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 4, num_threads)) untied
    ...
    T_Add4(B11, B21, T4, x);
    FastMatmulRecursive(A52, T4, M4, total_steps, steps_left - 1, (start_index + 4 - 1) * 18, x, num_threads);
    T4.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 5, num_threads)) untied
    ...
    T_Add5(B11, B12, T5, x);
    FastMatmulRecursive(S5, T5, M5, total_steps, steps_left - 1, (start_index + 5 - 1) * 18, x, num_threads);
    S5.deallocate();
    T5.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 6, num_threads)) untied
    ...
    T_Add6(B12, B22, T6, x);
    FastMatmulRecursive(A41, T6, M6, total_steps, steps_left - 1, (start_index + 6 - 1) * 18, x, num_threads);
    T6.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 7, num_threads)) untied
    ...
    T_Add7(B11, B12, T7, x);
    FastMatmulRecursive(S7, T7, M7, total_steps, steps_left - 1, (start_index + 7 - 1) * 18, x, num_threads);
    S7.deallocate();
    T7.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 8, num_threads)) untied
    {
#endif
    FastMatmulRecursive(A31, B12, M8, total_steps, steps_left - 1, (start_index + 8 - 1) * 18, x, num_threads);
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 9, num_threads)) untied
    ...
    FastMatmulRecursive(S9, T9, M9, total_steps, steps_left - 1, (start_index + 9 - 1) * 18, x, num_threads);
    S9.deallocate();
    T9.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 10, num_threads)) untied
    ...
    FastMatmulRecursive(S10, T10, M10, total_steps, steps_left - 1, (start_index + 10 - 1) * 18, x, num_threads);
    S10.deallocate();
    T10.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 11, num_threads)) untied
    ...
    FastMatmulRecursive(S11, B21, M11, total_steps, steps_left - 1, (start_index + 11 - 1) * 18, x, num_threads);
    S11.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 12, num_threads)) untied
    ...
    FastMatmulRecursive(S12, B22, M12, total_steps, steps_left - 1, (start_index + 12 - 1) * 18, x, num_threads);
    S12.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 13, num_threads)) untied
    ...
    FastMatmulRecursive(S13, B22, M13, total_steps, steps_left - 1, (start_index + 13 - 1) * 18, x, num_threads);
    S13.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 14, num_threads)) untied
    ...
    FastMatmulRecursive(S14, T14, M14, total_steps, steps_left - 1, (start_index + 14 - 1) * 18, x, num_threads);
    S14.deallocate();
    T14.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 15, num_threads)) untied
    ...
    FastMatmulRecursive(S15, B11, M15, total_steps, steps_left - 1, (start_index + 15 - 1) * 18, x, num_threads);
    S15.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 16, num_threads)) untied
    ...
    FastMatmulRecursive(S16, T16, M16, total_steps, steps_left - 1, (start_index + 16 - 1) * 18, x, num_threads);
    S16.deallocate();
    T16.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 17, num_threads)) untied
    ...
    FastMatmulRecursive(A32, B22, M17, total_steps, steps_left - 1, (start_index + 17 - 1) * 18, x, num_threads);
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp task if(should_launch_task(18, total_steps, steps_left, start_index, 18, num_threads)) untied
    ...
    FastMatmulRecursive(S18, B11, M18, total_steps, steps_left - 1, (start_index + 18 - 1) * 18, x, num_threads);
    S18.deallocate();
#if defined(_PARALLEL_) && (_PARALLEL_ == _BFS_PAR_ ||  _PARALLEL_ == _HYBRID_PAR_)
    }
...
# pragma omp taskwait
    ...
    M_Add1(M1, M3, M11, M15, C11, x);
    M_Add2(M5, M8, M12, M14, M15, C12, x);
    M_Add3(M1, M3, M9, M11, M14, M16, C21, x);
    M_Add4(M8, M12, M16, C22, x);
    M_Add5(M1, M3, C31, x);
    M_Add6(M8, M17, C32, x);
    M_Add7(M2, M4, M10, M13, C41, x);
    M_Add8(M6, M13, C42, x);
    M_Add9(M4, M18, C51, x);
    M_Add10(M2, M6, M7, M18, C52, x);
